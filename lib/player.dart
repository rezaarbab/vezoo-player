...
class _PlayerState extends State<PlayerScreen>{
  ...
  Future<void> _start()async{
    ...
    if(saved.inSeconds>5&&mounted){
      final resume=await showDialog<bool>(
        ...
        builder:(ctx)=>AlertDialog(backgroundColor:const Color(0xFF1D1220),title:Text(L.continuePlaying),
  ...
  void _handleVoskFinal(String t) {
    ...
    final startT = _lastFinalTime != null
        ...
        : (endT > const Duration(milliseconds: 500)
            ? endT - const Duration(milliseconds: 500) : Duration.zero);
    _lastFinalTime = now;
  ...
  void _toggleDeeepgram()async{
    ...
    if(false && _dgActive){
      ...
      _geminiDubMode = !(result['geminiSubMode'] as bool? ?? false);
      _geminiShowSub = result['geminiShowSub'] as bool? ?? false;
  ...
  void _showLogDialog(){
    showDialog(context:context,barrierColor:Colors.black54,builder:(ctx)=>StatefulBuilder(
      builder:(ctx,ss)=>AlertDialog(
        ...
        contentPadding:const EdgeInsets.all(12),
        title:Row(children:[
          const Icon(Icons.terminal_rounded,color:Color(0xFFA26592),size:18),
          const SizedBox(width:8),
          ...
          child:_liveLog.isEmpty
            ...
            :ListView.builder(
              ...
              itemBuilder:(_,i){
                ...
                final color=log.contains('▶')?const Color(0xFF7BC49A):
  ...
  void _showSleepDialog(){
    ...
    showDialog(context:context,builder:(ctx)=>StatefulBuilder(builder:(ctx,ss)=>AlertDialog(
      backgroundColor:const Color(0xFF1D1220),title:Text(L.sleepTimer),
      ...
      content:Column(mainAxisSize:MainAxisSize.min,children:[
        ...
        if(_subtitleTracks.isEmpty)Padding(
          ...
          child:Text(L.noEmbeddedSubtitle,
              style:TextStyle(color:Color(0xFFB4A3B4)),textAlign:TextAlign.center)),
        ..._subtitleTracks.asMap().entries.map((entry){
          final t=entry.value;
          return ListTile(dense:true,
            leading:const Icon(Icons.subtitles,size:18,color:Color(0xFFB4A3B4)),
        ...
        Padding(padding:EdgeInsets.all(8),
          child:Text(L.bothAtOnce,
              style:TextStyle(fontSize:11,color:Color(0xFFA26592)))),
      ]),
  ...
  void _showAudioPicker(){
    ...
    showDialog(context:context,builder:(ctx)=>AlertDialog(
      backgroundColor:const Color(0xFF1D1220),title:Text(L.selectAudioTrack),
  ...
  void _showVideoInfo(){
    showDialog(context:context,builder:(ctx)=>AlertDialog(
      ...
      content:Column(mainAxisSize:MainAxisSize.min,crossAxisAlignment:CrossAxisAlignment.start,children:[
        if(_resStr.isNotEmpty)_infoRow(Icons.aspect_ratio_rounded,const Color(0xFFA78BC0),L.resolution,_resStr),
        if(_fpsStr.isNotEmpty)_infoRow(Icons.speed_rounded,const Color(0xFFA26592),L.frameRate,_fpsStr),
        if(_codecStr.isNotEmpty)_infoRow(Icons.code_rounded,const Color(0xFF7BC49A),L.codec,_codecStr),
        if(_bitrateStr.isNotEmpty)_infoRow(Icons.network_check_rounded,const Color(0xFFE2A65B),L.bitrate,_bitrateStr),
        ...
        if(_buffering&&widget.isLive)Positioned(
          ...
          child:Container(
            ...
            child:Center(child:Column(mainAxisSize:MainAxisSize.min,children:[
              const CircularProgressIndicator(color:Color(0xFFA26592),strokeWidth:3),
        ...
        if(_isHDR)_infoRow(Icons.hdr_on_rounded,const Color(0xFFC76B93),'HDR',L.activeTick),
        if(_pixelFmtStr.isNotEmpty)_infoRow(Icons.palette_rounded,const Color(0xFFA26592),'Pixel Format',_pixelFmtStr),
        _infoRow(Icons.timer_outlined,const Color(0xFFB4A3B4),L.duration,fmt(_duration)),
        _infoRow(Icons.memory_rounded,const Color(0xFFB4A3B4),L.decoder,_hwDecode?L.hwDecode:L.swDecode),
        if(_audioTracks.isNotEmpty)
          _infoRow(Icons.music_note_rounded,const Color(0xFFB4A3B4),L.audioTracks,'${_audioTracks.length} ${L.audioTracks}'),
        if(_hwDecode)_infoRow(Icons.developer_board_rounded,const Color(0xFFA78BC0),L.decoder,L.hwActive),
      ]),
      actions:[TextButton(onPressed:()=>Navigator.pop(ctx),child:Text(L.close))],
    ));
  }
  ...
  Widget _infoRow(IconData icon,Color iconColor,String label,String val)=>Padding(
    ...
    child:Row(children:[
      ...
      Text('$label',style:const TextStyle(color:Color(0xFFB4A3B4),fontSize:12)),
      const Spacer(),
  ...
  Future<void> _startLiveSub(LiveSubConfig config) async {
    ...
    transcribeLive(
      ...
      if (mounted) {
        ...
        showSnack(context, L.liveDone, color: Color(0xFFA26592));
      }
    }).catchError((e) {
      if (mounted) {
        setState(() => _liveSubActive = false);
        ...
        if (!e.toString().contains(L.cancel)) {
          ...
          showDialog(context:context, builder:(_)=>AlertDialog(
            ...
            actions:[TextButton(onPressed:()=>Navigator.pop(context), child:const Text('بستن', style:TextStyle(color:Color(0xFFA26592))))],
          ));
        }
      }
    });
                ...
                child: Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    const Padding(padding: EdgeInsets.only(left: 10, top: 6, bottom: 6),
                      child: Icon(Icons.translate, color: Color(0xFFA26592), size: 12)),
                    const SizedBox(width: 5),
                    Padding(
        ...
        child:Row(children:[
          ...
          Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,mainAxisSize:MainAxisSize.min,children:[
              ...
              if(_isHDR||_resStr.isNotEmpty||_fpsStr.isNotEmpty)
                Row(children:[
                  if(_isHDR)_infoBadge('HDR',const Color(0xFFC76B93)),
                  if(_resStr.isNotEmpty)...[const SizedBox(width:4),_infoBadge(_resStr,const Color(0xFFA78BC0))],
                  if(_fpsStr.isNotEmpty)...[const SizedBox(width:4),_infoBadge(_fpsStr,const Color(0xFFA26592))],
                ]),
            ])),
          ...
          if(_subtitleTracks.isNotEmpty||_embeddedSubEnabled)
            GestureDetector(
              ...
              child:Container(
                ...
                decoration:BoxDecoration(
                  color:_embeddedSubEnabled?const Color(0xFFA26592):const Color(0xFFA26592).withOpacity(0.15),
                  borderRadius:BorderRadius.circular(6),
                  border:Border.all(color:const Color(0xFFA26592).withOpacity(_embeddedSubEnabled?0:0.4)),
                ),
                child:Icon(Icons.subtitles_outlined,size:16,
                    color:_embeddedSubEnabled?Colors.white:const Color(0xFFA26592)),
              ),
            ),
          PopupMenuButton<String>(
            icon:const Icon(Icons.subtitles,color:Color(0xFFA26592)),
            tooltip:L.subtitle,
            onSelected:(v){
              switch(v){
                case 'ai':
                  AiSubtitleSheet.show(context,_curPath,(srt){
                    ...
                    showSnack(context, L.aiSubtitleLoaded, color: Color(0xFFA26592));
                  },onPreview:(srt){
                    _loadSub(srt,secondary:false);
                  });
                  break;
                case 'online':
                  OpenSubtitlesSheet.show(context,_curPath,(srt){
                    _loadSub(srt,secondary:false);
                    showSnack(context, L.onlineSubtitleLoaded, color: Color(0xFFA26592));
                  });
                  break;
                case 'lyrics':
                  ...
                  if (_translating && SrtTranslationService.isRunning) {
                    ...
                    SrtTranslateSheet.show(
                      ...
                      (translated) {
                        ...
                        showSnack(context,
                          'ترجمه ذخیره شد:\n$translated',
                          color: Color(0xFFA26592), seconds: 10);
                      },
                      onDoneSecondary: (translated) {
                        if(!mounted) return;
                        setState((){_translating=false; _translatingStatus='';});
                        _loadSub(translated, secondary: true);
                        showSnack(context, L.translationOnSub2, color: Color(0xFFA26592), seconds: 10);
                      },
                ...
                case 'settings':
                  showModalBottomSheet(
                    context:context,isScrollControlled:true,backgroundColor:const Color(0xFF1D1220),
                    shape:const RoundedRectangleBorder(borderRadius:BorderRadius.vertical(top:Radius.circular(20))),
            ...
            itemBuilder:(_)=>[
              ...
              PopupMenuItem(value:'ai',child:Row(children:[
                Icon(Icons.auto_awesome,size:18,color:Color(0xFFA26592)),SizedBox(width:10),Text(L.aiSubtitleOffline),
              ])),
              PopupMenuItem(value:'online',child:Row(children:[
                Icon(Icons.cloud_download_outlined,size:18,color:Color(0xFFA26592)),SizedBox(width:10),Text(L.onlineSubtitleLabel),
              ])),
              if(_sub1Path!=null)
                PopupMenuItem(value:'translate',child:Row(children:[
                  Icon(Icons.translate,size:18,color:Color(0xFFA26592)),SizedBox(width:10),Text(L.translateSub),
                ])),
                PopupMenuItem(value:'lyrics',child:Row(children:[
                  Icon(Icons.music_note_rounded,size:18,color:Color(0xFFC76B93)),SizedBox(width:10),Text(L.musicSubtitle),
                ])),
          ...
          GestureDetector(
            ...
            child:AnimatedContainer(
              ...
              decoration:BoxDecoration(
                ...
                boxShadow:[BoxShadow(
                  color:_playing?Colors.black.withOpacity(0.35):const Color(0xFFA26592).withOpacity(0.45),
    ...
    return Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      if (_useGeminiLive) ...[
        GestureDetector(
          ...
          child: Container(
            padding: const EdgeInsets.symmetric(horizontal:8, vertical:4),
            decoration: BoxDecoration(color: const Color(0xFF7BC49A).withOpacity(0.8), borderRadius: BorderRadius.circular(6)),
            child: const Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.record_voice_over_rounded, size:12, color:Colors.white),
              SizedBox(width:3),
      ...
      if (_useVosk || _useAndroidStt) ...[
        GestureDetector(
          ...
          child: Container(
            padding: const EdgeInsets.symmetric(horizontal:8, vertical:4),
            decoration: BoxDecoration(color: const Color(0xFFA26592).withOpacity(0.8), borderRadius: BorderRadius.circular(6)),
            child: const Row(mainAxisSize: MainAxisSize.min, children: [
  ...
  Widget build(BuildContext ctx) {
    ...
    return SafeArea(child: SingleChildScrollView(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        const VzSheetHandle(),
        Row(children:[
          const Icon(Icons.translate,color:Color(0xFFA26592),size:18),
          const SizedBox(width:8),
        ...
        ValueListenableBuilder<int>(
          ...
          builder:(_,__,___) => Column(children:[
            Row(children:[
              const Icon(Icons.translate,color:Color(0xFFA26592),size:14),
            ...
            LinearProgressIndicator(
              ...
              backgroundColor:Colors.white12,color:const Color(0xFFA26592)),
            const SizedBox(height:4),
...
class _VoskSettingsDialogState extends State<_VoskSettingsDialog> {
  ...
  Widget build(BuildContext ctx) => AlertDialog(
    ...
    content: SizedBox(width: 300, height: 420, child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      Container(
        decoration: BoxDecoration(
          color: _geminiEnabled ? const Color(0xFF7BC49A).withOpacity(0.1) : const Color(0xFF241627),
          borderRadius: BorderRadius.circular(10),
          border: Border.all(color: _geminiEnabled ? const Color(0xFF7BC49A) : Colors.white12)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), child: Row(children: [
            const Icon(Icons.record_voice_over_rounded, size: 16, color: Color(0xFF7BC49A)),
            const SizedBox(width: 8),
            ...
            Switch(value: _geminiEnabled, onChanged: (v) => setState(() { _geminiEnabled = v; if (v) { _voskEnabled = false; _androidEnabled = false; } }), activeColor: const Color(0xFF7BC49A)),
          ])),
          if (_geminiEnabled) ...[
            ...
            Padding(padding: const EdgeInsets.fromLTRB(12, 8, 12, 4), child: Row(children: [
              Expanded(child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: !_geminiSubMode ? const Color(0xFF7BC49A).withOpacity(0.2) : const Color(0xFF241627),
                    border: Border.all(color: !_geminiSubMode ? const Color(0xFF7BC49A) : Colors.white12),
                    borderRadius: BorderRadius.circular(8)),
                  child: Column(children: [
                    Icon(Icons.record_voice_over_rounded, size: 18, color: !_geminiSubMode ? const Color(0xFF7BC49A) : Colors.white38),
                    const SizedBox(height: 4),
              ...
              Expanded(child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.circular(8)),
                  child: Column(children: [
                    Icon(Icons.subtitles_rounded, size: 18, color: _geminiSubMode ? const Color(0xFF7BC49A) : Colors.white38),
                    const SizedBox(height: 4),
            ...
            Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (!_geminiSubMode) Row(children: [
                ...
                Switch(value: _geminiShowSub, onChanged: (v) => setState(() => _geminiShowSub = v), activeColor: const Color(0xFF7BC49A)),
              ]),
              const SizedBox(height: 6),
              if (!_geminiSubMode) Row(children: [
      ...
      Container(
        decoration: BoxDecoration(
          color: _voskEnabled ? const Color(0xFFA26592).withOpacity(0.1) : const Color(0xFF241627),
          borderRadius: BorderRadius.circular(10),
          border: Border.all(color: _voskEnabled ? const Color(0xFFA26592) : Colors.white12)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), child: Row(children: [
            const Icon(Icons.subtitles_rounded, size: 16, color: Color(0xFFA26592)),
            const SizedBox(width: 8),
            const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Switch(value: _voskEnabled, onChanged: (v) => setState(() { _voskEnabled = v; if (v) _androidEnabled = false; }), activeColor: const Color(0xFFA26592)),
          ])),
          if (_voskEnabled) ...[
            const Divider(color: Colors.white12, height: 1),
            Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                ...
                Switch(value: _translate, onChanged: (v) => setState(() => _translate = v), activeColor: const Color(0xFFA26592)),
              ]),
              if (_translate) ...[
                const SizedBox(height: 8),
                ...
                Row(children: [
                  Expanded(child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: _useOffline ? const Color(0xFFA26592).withOpacity(0.15) : const Color(0xFF241627),
                        border: Border.all(color: _useOffline ? const Color(0xFFA26592) : Colors.white12),
                        borderRadius: BorderRadius.circular(10)),
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        const Row(children: [Icon(Icons.wifi_off_rounded, size: 13, color: Color(0xFFA26592)), SizedBox(width: 5), Text('Offline (ML Kit)', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12))]),
                        ...
                        if (_useOffline && _mlkitReady == true)
                          const Padding(padding: EdgeInsets.only(top:4), child: Text('✅ Model ready', style: TextStyle(color: Color(0xFF8BD4A8), fontSize: 9, fontWeight: FontWeight.bold)))
                          ...
                          TextButton(
                            ...
                            style: TextButton.styleFrom(foregroundColor: const Color(0xFFA26592), padding: EdgeInsets.zero, minimumSize: Size.zero, tapTargetSize: MaterialTapTargetSize.shrinkWrap),
              ...
              Row(children: [
                Expanded(child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: !_translateOnFinish ? const Color(0xFF7BC49A) : const Color(0xFF241627),
                      borderRadius: BorderRadius.circular(8)),
                    child: Column(children: [
      ...
      Container(
        decoration: BoxDecoration(
          color: _androidEnabled ? const Color(0xFFA78BC0).withOpacity(0.1) : const Color(0xFF241627),
          borderRadius: BorderRadius.circular(10),
          border: Border.all(color: _androidEnabled ? const Color(0xFFA78BC0) : Colors.white12)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), child: Row(children: [
            const Icon(Icons.android_rounded, size: 16, color: Color(0xFFA78BC0)),
            const SizedBox(width: 8),
            const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Switch(value: _androidEnabled, onChanged: (v) => setState(() { _androidEnabled = v; if (v) _voskEnabled = false; }), activeColor: const Color(0xFFA78BC0)),
          ])),
          if (_androidEnabled) ...[
            const Divider(color: Colors.white12, height: 1),
            Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
