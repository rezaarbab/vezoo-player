...
// ─────────────────────────────────────────────────────────────────────────────
//  CLICK STYLE — افکت لمسی روی کارت‌ها/دکمه‌ها (به‌جای پت انیمه)
// ─────────────────────────────────────────────────────────────────────────────
enum VzClickStyle {
  /// موج کلاسیک Material که از نقطه‌ی لمس پخش می‌شود.
  ripple,
  /// فشرده‌شدن فنری (scale) با بازگشت کشسان.
  spring,
  /// بالا آمدن کارت + سایه‌ی بیشتر هنگام لمس.
  lift,
  /// درخشش (glow) اکسنت زیر عنصر.
  glow,
  /// هیچ افکت اضافه‌ای — فقط بازخورد لرزشی.
  none,
}

extension VzClickStyleLabel on VzClickStyle {
  String get label => switch (this) {
    VzClickStyle.ripple => 'موج (Ripple)',
    VzClickStyle.spring => 'فنری (Spring)',
    VzClickStyle.lift   => 'بالا آمدن (Lift)',
    VzClickStyle.glow   => 'درخشش (Glow)',
    VzClickStyle.none   => 'ساده',
  };
}
...
class VzThemeState extends State<VzTheme> with WidgetsBindingObserver {
  ...
  /// پیش‌بارگیری هم‌زمانی تم از prefs — در main قبل از runApp صدا می‌خورد
  ...
  Future<void> setClickStyle(VzClickStyle s) async {
    _click = s;
    setState(() {});
  ...
  return ThemeData(
    ...
    switchTheme: SwitchThemeData(
      ...
      trackColor: WidgetStateProperty.resolveWith(
        (s) => s.contains(WidgetState.selected) ? p.accent : p.cardHi),
    ...
    tabBarTheme: TabBarThemeData(
      indicatorColor: p.accent, labelColor: p.text,
    ...
    dialogTheme: DialogThemeData(
      backgroundColor: p.surface, surfaceTintColor: Colors.transparent,
    ...
    inputDecorationTheme: InputDecorationTheme(
      ...
      hintStyle: TextStyle(color: p.textDim, fontSize: 13),
      border: OutlineInputBorder(borderRadius: rSm, borderSide: BorderSide(color: p.border)),
      enabledBorder: OutlineInputBorder(borderRadius: rSm, borderSide: BorderSide(color: p.border)),
      focusedBorder: OutlineInputBorder(borderRadius: rSm, borderSide: BorderSide(color: p.accent, width: 1.4)),
    ),
    popupMenuTheme: PopupMenuThemeData(
      color: p.cardHi, surfaceTintColor: Colors.transparent,
      shape: RoundedRectangleBorder(borderRadius: rSm),
    ),
    ...
    outlinedButtonTheme: OutlinedButtonThemeData(
      style: OutlinedButton.styleFrom(
        foregroundColor: p.text, side: BorderSide(color: p.borderHi),
        shape: RoundedRectangleBorder(borderRadius: rSm)),
    ),
    ...
    snackBarTheme: SnackBarThemeData(
      ...
      shape: RoundedRectangleBorder(borderRadius: rSm, side: BorderSide(color: p.border)),
      insetPadding: const EdgeInsets.fromLTRB(16, 0, 16, 90),
    ),
    tooltipTheme: TooltipThemeData(
      decoration: BoxDecoration(color: p.cardHi, borderRadius: BorderRadius.circular(8)),
      textStyle: TextStyle(fontSize: 11, color: p.text),
    ),
    progressIndicatorTheme: ProgressIndicatorThemeData(
      color: p.accent, linearTrackColor: p.cardHi),
    iconTheme: IconThemeData(color: p.text),
    textTheme: TextTheme(
      titleMedium: TextStyle(fontWeight: FontWeight.w600, letterSpacing: -0.2, color: p.text),
      bodySmall: TextStyle(color: p.textSec),
    ),
  );
}
